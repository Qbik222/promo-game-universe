...
## Анімація відкриття коробки та показу призу

### 1. Відкриття коробки (`initOpenBox`)
- Запускається анімація коробки `shakeUntil`.
- Викликається `getUserPrize()` — симуляція отримання призу з API.
- Коробка відкривається на 2 сек і закривається.
- Далі запускається прелоадер.

### 2. Прелоадер (`showPreloader`)
- Відкривається попап прелоадера.
- Показується потрібний приз.
- Картки з’являються послідовно з інтервалом 250 мс.
- Через ~1.25 сек картка перевертається (`is-flipped`).
- Ще через 2 сек стан прелоадера очищується.
- Через 0.8 сек відкривається попап з виграшем.

### 3. Службові функції
- `openPopupByAttr(attr)` — відкриває попап за атрибутом.
- `showPrizeByDataAttr(prize)` — показує потрібний приз.
- `clearPreloaderState()` — скидає стани карток і ховає прелоадер.

---

### Примітка про `getUserPrize`
Зараз `getUserPrize` повертає тестове значення (`Promise.resolve('prize1')`).  
У майбутньому ця функція має отримувати дані з бекенду про те, який саме приз випав користувачу.  
Формат відповіді з бекенду поки невідомий, тому зроблено спрощене рішення через текстовий атрибут.  
Коли бек буде готовий — тут потрібно буде доопрацювати логіку обробки реальних даних.

---

**Процес:** відкриття коробки → отримання призу → прелоадер з анімацією → фінальний попап з виграшем.
